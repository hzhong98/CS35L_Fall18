...
void *process(void *id) {
	...
	for( int py=0; py<height; ++py )
	{
	    ...
	    Pxl pixel;
	    pixel.r = scaled_color[0];
	    pixel.g = scaled_color[1];
	    pixel.b = scaled_color[2];
	    imgPxls[px][py] = pixel;
	}
    }
    return NULL;
...
main( int argc, char **argv )
{
    ...
    {
	id[ii] = ii;
	if (pthread_create(&thread[ii], NULL, process, &id[ii]))
	{
	    fprintf(stderr, "Error: Bad Thread");
	    exit(1);
	}
    }
    for (ii = 0; ii < nthreads; ii++)
	...
	{
	    printf("%.0f %.0f %.0f\n", imgPxls[ii][jj].r, imgPxls[ii][jj].g, imgPxls[ii][jj].b);
	}
	printf("\n");
    }
